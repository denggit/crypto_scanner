...
class BatchFastBacktest:
    ...
    def run_batch_backtest(self, min_vol_ccy: float = 1000000, limit: int = 300):
        """运行批量回测"""
        logger.info(f"开始批量回测，筛选24小时交易量 >= {min_vol_ccy:,.0f} USDT的币种")
        
        try:
            # 获取高交易量币种
            symbols = self.scanner._get_volume_filtered_symbols('USDT', min_vol_ccy, use_cache=True)
            ...
            for i, symbol in enumerate(symbols, 1):
                ...
                report = self.run_single_backtest(symbol, limit)
                if report:
