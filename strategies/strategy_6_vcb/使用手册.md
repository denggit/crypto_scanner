...
| 函数名 | 作用 | 调用时机 |
|--------|------|----------|
| `calculate_stop_loss()` | 计算硬止损价格 | 开仓时 |
...
1. **硬止损**：基于压缩区间另一侧和ATR(60)兜底
2. **主动止盈**：支持R倍止盈、布林中轨止盈、对侧轨道止盈、ATR跟踪止盈
3. **失败退出**：突破后X根K线内无法扩展波动率则平仓
4. **Break-even**：浮盈≥1R时，止损移到入场价
...
```json
{
  ...
  "stop_loss_atr_multiplier": 0.8, // 止损ATR倍数（硬止损的ATR兜底）
...
| `stop_loss_atr_multiplier` | float | 0.8 | 止损ATR倍数（硬止损的ATR兜底） |
...
| `failure_exit_bars` | int | 10 | 失败退出K线数量（突破后X根K线内无法扩展波动率则平仓） |
...
**执行顺序**（按优先级）：
1. **硬止损**（最高优先级）- 一旦触发立即平仓
2. **Break-even**（更新止损）- 不触发平仓，只更新止损价格
3. **主动止盈** - 根据止盈模式检查
4. **失败退出** - 时间成本控制

**注意**：每个持仓每根K线都会按此顺序检查，一旦触发平仓条件，立即执行。

### Q13: 如何理解"硬止损"和"失败退出"的区别？
